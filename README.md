# 📅 MyRoomScheduler (공유 자원 예약 시스템) - v1.2
...
* **v1.2 (Update):**
    * ✅ **중복 체크 :** 데이터베이스를 조회하여 기존 예약과 1분이라도 겹치면 등록을 차단합니다.
    * ✅ **간편한 UI 전환 :** 일일이 시간을 타이핑하던 방식에서 마우스 클릭만으로 선택 가능한 드롭다운 방식으로 개선되었습니다.
    * ✅ **실기한 피드백 :** 예약 성공/실패 시 사용자에게 즉각적인 팝업 알림을 제공합니다.
...
* **Build Tool:** Gradle
* **IDE:** IntelliJ IDEA
* **Database**: SQLite (JDBC 연결)
* **Algorithm**: 시간 오버랩 비교 로직 (`(StartA < EndB) && (EndA > StartB)`)
...
#### 🔥 Issue 5: 예약 시간 중복 등록 및 입력 불편함
**문제**: 형과 내가 같은 시간에 방을 예약하려 할 때, 시스템이 이를 걸러내지 못하고 중복 저장되는 현상 및 매번 시간을 직접 타이핑해야 하는 번거로움 발생.

**원인**: 기존 로직은 데이터 존재 여부 확인 없이 INSERT만 수행했으며, JTextField 사용으로 인해 시간 포맷(: 또는 ~) 오타 시 런타임 에러 위험이 있었음.

**해결**: * 로직 개선: isOverlapping 메서드를 추가하여 (새 시작 < 기존 종료) && (새 종료 > 기존 시작) 논리로 시간 겹침을 사전 차단.

---

### ⚠️ 향후 개선 계획 (Roadmap)
실사용 테스트를 통해 발견된 점들을 **v2.0**에서 보완할 예정입니다.
* **디테일한 오류 방지(시간 역전 막기)**: 시작 시간 16:00, 종료 시간 14:00으로 거꾸로 설정하지 못하게 보완.
* **실행파일 생성**: 바탕화면에서 더블클릭만 하면 바로 실행되도록 독립적인 실행 파일 생성.
